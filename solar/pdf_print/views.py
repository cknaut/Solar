...
def index(request):
    ...
    Tend = 20000
    nsteps = 2000 
    return print_pdf(request, names, ms, qs, vs, Tend, nsteps)
    


def print_pdf(request, names, ms, qs, vs, Tend, nsteps):
    ...
    for i in range(Solar.n_planets):
        ...
        ax.plot(q_planet[:, 0], q_planet[:, 1], "-", label=Solar.planets[i].name)
